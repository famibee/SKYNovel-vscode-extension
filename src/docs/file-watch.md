...
**#3・#4（mac (VE)→(VE)）で全走査「0」を実測済み（2026-09-13）。** 要求層の設計
（「LSP全走査は`.sn`の存在変化が契機、画像/音声の追加だけでは起きない」14-25行目の表）
を裏付ける結果。`test/ui/runUI.ts` の D&D 各ケースに `need_go.send` の差分ログを追加して計測。

※ **#1・#2・#5（Finder が絡む3ケース）は今回このセッションからの実行では
未計測。** `System Events` が Finder ウィンドウを掴めず（`-1719`）自動化コード自体が
失敗した。**「このセッション自体が VSCode 上で動く Claude Code の子プロセスとして
実行されている」環境要因**（本節・testing.md に既知の制約として記載済み）による
ものとみられ、コード変更が原因ではなさそう。プレーンターミナルからの再実行が必要。

win 側（#7〜#12）は `need_go.send` 計測コードが未展開（mac 側のこの変更は
まだ commit/push 前で win 側リポジトリに同期されていない）。Win 担当セッションへ
展開を依頼するかは要検討。
