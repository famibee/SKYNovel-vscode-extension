...
| # | 内容 | 時期 | 節 |
|---|---|---|---|
| 1 | **Marketplace 再申請**（新 ID `famibee2.bluesnovel`） | **8/23 以降**。それまでメールを送らない | §1 |
...
| # | 内容 | なぜ後か | 節 |
|---|---|---|---|
| 1 | **マルチルートで static が後勝ち** | **最重**。別プロジェクトの設定で暗号化しかねない。単独の版で | §3.8(A) |
| 2 | 500ms のまとめが監視ごと | 1 と逆行するので 1 が先 | §3.8(B) |
| 3 | `updPathJson()` が3役 | 分割 | §3.8(C) |
| 4 | **再検証するファイルを絞る**（73.6ms・75%。天井は 1/8〜1/10） | 絞り込みを誤ると**誤診断が残る／消えない** | §3.7 宿題3 |
| 5 | **マルチルートの残り5件**（LSP とツリーが解放されない／閉じたのと別の行が消える／まとめて追加できない／前方一致で誤配）＋**リソース解放5件**（監視9本が捨てられていない ほか） | 1 と同じ関数群を触るので**まとめて**。**済んだら `keywords` に `multi-root ready` を戻す**（2026-07-29 に外した） | §3.6 |
