...
export class BufferTransform extends Transform {
	...
	private	cnt_code	= BufferTransform.LEN_ENC;
	private	ite_buf		= 2;
	...
	constructor(readonly short_path: string, private readonly ws: WriteStream) {
	...
	private transform(chunk: any, _enc: any, cb: (error?: Error | null, data?: any) => void): void {
		...
		const e = Buffer.from(
			String(lib.WordArray.create(Array.from(this.bh))), 'base64',
