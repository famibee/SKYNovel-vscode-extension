...
✅ **【再検証・2026-09-14】仮説は再現せず。** Win側セッションで
`bun run test:int` を3回連続実行したが、**毎回8回の全走査が安定して発火**
（中央値62.3ms、他2回もほぼ同値）。`sleep(6000)` は変更していない。
テスト側は変えていないので、**このドキュメント執筆時点からこの日までに
入った他の修正（9/13 のマルチルート対応一式・`WorkSpaces.ts` `#refresh()`
書き直し等）が副次的に解消した**とみられるが、根本原因の特定はできていない。
現状は安定して発火するため、**この件はクローズ**（再発したら本節に戻る）。
