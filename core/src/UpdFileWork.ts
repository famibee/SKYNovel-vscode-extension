...
import {uint, foldProc} from './CmnLib';
...
export	function updPathJson(cur: string) {
	if (! fs.existsSync(cur +'prj.json')) return;
...
export	function updPlugin(curPlg: string) {
	if (! fs.existsSync(curPlg)) return;

	const h: any = {};
	foldProc(curPlg, ()=> {}, nm=> h[nm] = 0);
...
//	const REG_FN_RATE_SPRIT	= /(.+?)(?:%40(\d)x)?(\.\w+)/;
	...
	for (const dir of oCfg.search) {
		...
		foldProc(wd, (url, nm)=> {
			...
			if (! m) {addPath(hFn2Path, dir, nm); return;}
