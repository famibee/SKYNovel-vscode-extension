...
それで、**フォルダ追加削除は合成イベントを minimatch でそこへ流し込んでいる**
（リネームは (D) の対応により FS 監視の del+cre だけで届くので合成不要になった）。
つまり半分はこのモデルになっている。足りないのは、生の FS イベントがレジストリを
通らず監視ごとに直結していることと、横断的な関心（path.json・暗号化）が
boolean フラグで外付けされていること。

| # | 理想 | 現状 | 対応 |
|---|---|---|---|
| 1 | 状態はプロジェクトごと | **済**（`PrjCmn` インスタンスフィールド化・2026-09-13） | **(A)** |
| 2 | まとめはプロジェクト単位 | **済**（`PrjCmn` インスタンスフィールド化・2026-09-13） | **(B)** |
| 3 | 要求は独立 | **済**（`#updPathJsonCore` / `#updDropCandidates` / `#updPathJson` に分割・2026-09-13） | **(C)** |
...
| 7 | フォルダ・リネームも同じ口 | **既にそう**（フォルダは合成イベントを replay。リネームは FS 監視の del+cre がそのまま届く） | ✅ |
...
#### ✅ (B) 500ms のまとめが監視インスタンスごと【決着・実装済み・2026-09-13】

**何が困っていたか：** **画像と音声を同時にドロップする**と、`loadEx`
（＝プロジェクト全体の走査＋暗号化）が**2回走っていた**。1回で済むはずの重い処理が倍になる。

**なぜそうなっていたか：** ファイル監視は**パターンごとに別インスタンス**（`{jpg,png}` 用、
`{mp3,wav}` 用…と9本ある）。「立て続けの変化を 500ms まとめて1回にする」という
待ち合わせが、**その1本ずつに付いていた**。⇒ 画像用と音声用が**別々に**
500ms 数えて、**別々に** `updPathJson()` を呼んでいた。

```
旧          画像監視 ─500ms→ updPathJson()   ← 2回走る
            音声監視 ─500ms→ updPathJson()
直した後    画像監視 ┐
            音声監視 ┴500ms→ updPathJson()   ← 1回
```

**直し方：** 待ち合わせ（`#tiLasyPathJson` とその管理）を `WatchFile`
インスタンスから `PrjCmn`（`this.pc`。プロジェクトに1個。(A) で導入した置き場）
へ移した。`WfbOptPic` / `WfbOptSnd` / `WfbOptFont` のどれから呼んでも
同じタイマーを共有するので、種類が違っても1回にまとまる。

統合テスト（`画像と音声を同時に追加しても updPathJson は1回・全走査も1回`）で
`need_go.req` が 2→1 になったことを確認済み。

#### ✅ (C) `updPathJson()` が3役【決着・実装済み・2026-09-13】

1つの関数が **path.json 更新＋暗号化／ドロップ先候補の更新／LSP への全走査要求**を
兼ねていた。画像を1枚置いただけで前2つは必ず走る（3つめは元々 path.json が
実際に変わったときだけという条件付き）。

`Project.ts` の3つを独立したメソッドに分割：`#updPathJsonCore()`（path.json
更新＋暗号化）／`#updDropCandidates()`（ドロップ先候補の再計算）／
`#updPathJson()`（上2つを呼び、path.json の差分を見て `#sendNeedGo()` を呼ぶ
オーケストレーター）。実行タイミングやトリガー条件は変更していない
（要求層の理想形どおり「ドロップ先候補は path.json と同時」のまま）。
狙いは処理のタイミングを変えることではなく、**1つの関数が3つの責務を
黙って兼ねている状態を解消し、それぞれの役割をコードから読み取れるようにする**こと。

#### ✅ (D) エディタ主導の**変名だけ**で購読者が二重に呼ばれる【決着・実装済み・2026-09-13】

変名の扱い自体は良い設計。**del + cre に分解**し、判定を「対（旧,新）」ではなく
**辺ごと**に独立させているので、4通りが2つの if で尽きる（組み合わせが増えない）。

しかし実測すると、**経路が2つあって重なっていた**：
...
`w.crechg` / `w.del` を呼んでいた。**画像最適化と暗号化が2回走る。**
`need_go` はデバウンスで1回に見えるので**外からは気づけない**。

対処は2案あったが、**案2（FS 監視だけで足りるので `#onDidRenameFiles` を消す）
を採用**。mac・Windows 両方の実測で「変名は del+cre だけで拾える」ことを
確認済みだったため（`workspace.onDidRenameFiles` の購読自体を削除）。

修正後、`test/int/suite.ts` の変名テストで (1)(2) 経路が完全に同一の結果
（`watch.cre:1 watch.del:1`、`watch.rename` は発火しない）になることを確認済み。
...
⚠️ **【2026-09-14・検証試行】Win側の開発機（シングルモニタ環境）では
検証不可だった。** マルチモニタ環境の用意ができ次第、改めて確認すること。
