...
- style			CSS style（一部）	文字ボタン上にマウスカーソルが乗っていない状態のCSS スタイル。
- style_hover			CSS style（一部）	文字ボタン上にマウスカーソルが乗っている状態（クリックしていない）のCSS スタイル。<br/>設定できる値はstyleと同様。
- style_clicked			CSS style（一部）	文字ボタン上にマウスカーソルが乗っている状態（クリックしている）のCSS スタイル。<br/>設定できる値はstyleと同様。
