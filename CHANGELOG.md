...
- perf: **vsix を 4095KB → 2995KB に（-1.1MB）**
...
## v4.27.3
- fix: 暗号化時に doc_crypto/app/icon.png の準備をしていなかったのを修正
## v4.27.2
...
- feat: バッチ処理方針変更。なるべく本体側で行い、最小限の実処理のみ委託するように
	...
	- sharp は macOS/win・CPU arch など環境ごとのインストールが必要で、どうしてもバッチ処理として外部に出さざるを得ない
	- feat: 音声最適化は本体側でできるようなので、非バッチ・内蔵化
	- feat: フォント最適化は Python コールだけなので、非バッチ・内蔵化
- fix: 主要機能の全体的な動作再確認
- fix: async / await 関連の不具合修正
- fix: 開いてないドキュメントでの NFD 文字警告の行列位置がダミー値だった件
- fix: 画像ファイル名にマウスホバーしたさい、画像が表示されない件
- fix: 画像・音声最適化で大量のファイルを処理するさい、数個処理漏れがある件
	- p-queue 不使用と async / await 関連の不具合修正による
	- 画像最適化で 331 個近いファイルで発生
	- 音声最適化で 90 個近いファイルで発生
- fix: ライブラリ更新
	- 廃止・不使用に
		- await-notify
		- p-queue
	- 内蔵に
		...
		- psd.js
	- ライブラリ変更
		- string-replace-async -> str-async-replace
			- メンテが四年前 -> 二年前、多機能につき
...
- fix: 暗号化 ON状態で、最適化すると暗号化出力が消える件
	- 暗号化 ON
		...
		- ❌ 音声最適化 ->OFF
			- 削除する
			- ❌（src/prj_base から復帰した）非最適化ファイルから暗号化ファイルを生成すべき
	- 暗号化 OFF
		- ✅ 音声最適化 ->ON
		- ✅ 音声最適化 ->OFF
...
## v4.20.5
- fix(src/Project.ts): 画像・音声ファイルの追加削除でありません系エラーが変化しない件
## v4.20.4
...
## v4.18.1
- fix(立ち絵素材生成機能): スクリプトで不要な【&const.ae.立ち絵.〜】記述を削除
## v4.18.0
- feat: core/resource/ に psd ファイルを置くと、doc/prj/face/ 下に立ち絵表示が可能となる素材を生成する機能
	- レイヤごとに 透過png として出力
	- ただし一番下のレイヤまたはフォルダ内レイヤは psd キャンバスサイズの 透過png として出力
	- 必要な [add_face] を羅列した sn スクリプト
- test: esbuild-jest がメンテされてないので ts-jest に切り替え
- fix: ターミナル系処理のポイント文字をカラフルに
- fix: リファクタリング
## v4.17.1
- fix: 仮に NFC 変換しても変化がない NFD 文字は警告しないように
- fix: 開いてないテキストでエラー位置は仮のままだが、開いたら再走査するように
## v4.17.0
- feat: 文字の Unicode正規化形式が NFD の場合に警告するように
- feat: NFD 警告のマウスオーバーで NFC に変換するコードアクション追加
	- クリックするだけで修正する機能
- fix(prj.json): debug 部の dumpHtm 記述抜けを追加
## v4.16.11
...
- docs(タグ説明): [lay] に filter・enable_filter 属性追加
...
- fix(設定画面): アプリタブで入力すると、基本情報タブで入力した値が古いモノになってしまう件
	- 原理的に逆もあり
	- （内部原因メモ）vue内部に二重化していた。vue扱う値のみを PrjSetting.ts に返すように
- refactor(src/PrjSetting.ts): リファクタリング
## v4.16.2
- fix(テンプレから新プロジェクト作成): 動作してなかったので修正
- fix(テンプレからプロジェクト更新): 動作してなかったので修正
## v4.16.1
- fix(組み込み変数): const.sn.fn2ext.json 追加
## v4.16.0
- feat: [macro]の引数説明記述で、値域・型について機能追加
	- （注釈）%layer?='レイヤ名|mes|処理対象の文字レイヤ' なら「レイヤ名」部分の件です
	- feat: 全角セミコロン「；」後は無視する。コメントを書ける
	- feat: 「一文字」追加。記述時チェックを行い、文字数が１でなければエラー通知
	- feat: 「整数」「実数」、後述の列挙型など追加（詳細は後述）。マクロ使用時チェックを行い、違反ならエラー通知
	- feat: 引数型の表記ゆれ統一と新規追加
		- 画像ファイル名、レイヤ名などのキーワード
			+ 存在しない場合にエラー通知
		- 一文字
			+ 文字数が１かどうかチェック
		- 整数
		...
		- 実数
			...
			- 【旧表記】正負の実数値 -> 実数
			- 【旧表記】横座標 -> 実数；横座標
				+ 「実数」が型表記、「；」以降はタグ入力者向け説明
		- 0.0〜1.0；透過度
			- -360.0〜360.0；角度
				+ 数値の範囲指定。下限上限を実数扱いチェック
		- 1〜
			+ 正の整数、上限なし。数値の範囲指定は上限を省略すると上限なしに
		- 0〜
			- 【旧表記】Integer -> 0〜
			- 【旧表記】ミリ秒数 -> 0〜；ミリ秒
				+ 非負整数、上限なし
		- fore、back
			- 【旧表記】Boolean -> true、false
			- num、int、uint、bool、str
				+ 列挙型
		- 色指定。0x000000など
			+ という「〜」「、」を含まない説明文字列を書くと、チェックなし
			- 【旧表記】String -> 文字列
				+ または具体的な内容説明に
		- [tsy]系の属性 -> 相対値か絶対値
			+ 説明文字列、チェックなし
	- feat: 値域・型の後ろに【（カンマ区切りで複数可）】を付加すると複数指定可能な引数となり、カンマ区切りで記述時チェックでエラーにならなくなる
		- すでに「レイヤ名（カンマ区切りで複数可）」はあった
		- （半角カンマ区切りで複数可能）->（カンマ区切りで複数可）
		- aaa、bbb（カンマ区切りで複数可）
			+ 列挙型で複数指定可能な場合
	- feat: （カンマ区切りで複数可）でエラーが出た際、どの項目でエラーになったか分かりやすいよう個別にエラー波線を引くように
- fix: [macro]の引数説明記述で、値域・型の表記ゆれ統一と新規追加
- refactor: タグ・マクロ引数チェックまわりのリファクタリング
- refactor: src/md2json.ts と入力の *.md で２セクション目をタブ区切りではなくバッククオート「`」区切りに変更、タブでは間違えやすいので
- テンプレ、拡張機能も随時、引数説明記述を拡張機能v4.16.0準拠に
## v4.15.0
...
- fix: 暗号化と画像・音声最適化での連携処理の全面再チェック・修正
	...
	- fix: 暗号化状態＆画像・音声最適化状態でそれらのファイルを更新時、その暗号化をしない件

	- fix: 暗号化状態で画像・音声最適化状態を切り替えると、切り替え前のファイルが暗号化フォルダに残る件
	- fix: 画像・音声素材やその最適化ファイル削除時、暗号化フォルダから削除していなかった件

	- fix: 画像変換画質変更による最変換時、一部ファイルが削除される件
		- ファイル変更監視の適切な一時停止がなされてなかった
	- fix: 音声変換エンコード変更による最変換時、一部ファイルが削除される件
		- ファイル変更監視の適切な一時停止がなされてなかった
	- fix: 暗号化状態で音声変換エンコード変更による最変換時、前エンコードの暗号化ファイルが残る件

	- fix: 画像・音声最適化状態切り替え時に path.json を再作成していなかった件
	- fix: 音声エンコーダー切り替え時に path.json を再作成していなかった件

	- fix: 画像・音声最適化状態を する にした際、diff.json にキーに余分な「base/」がついたものが追加される件
	- fix: jpg ファイル差異を検知しない場合があったため、ハッシュ精製バイト数を 1024→1024 *20 に拡大
	- fix: 単体音声ファイル最適化時、build/cnv_mat_snd.json がそのファイルひとつになってしまう件

	- fix: 画像・音声最適化バッチ、リファクタリング
	- fix: ファイル監視まわりの非同期・動機を厳格化・再検討
## v4.14.0
- feat: 【設定】【フォント情報】のフォント最適化処理で、フォントファイルに含まれない文字があった場合に警告表示するように
- feat: 【設定】【フォント情報】のフォント最適化処理のログファイルを確認できるように
## v4.13.1
- fix: 変数代入文字列をフォント生成対象とする機能、&代入で変数名文字列を対象としていたミス修正
- fix: 変数代入文字列をフォント生成対象とする機能、text属性が & 始まりの場合は変数名なので処理しないように
- fix: [lay]も[span]と同様に、font-family 確認でフォント切り替えするように
- fix: 半角空白を含むファイル名のフォント最適化に失敗していた件
- fix: core/font/font.json の環境値を塗りつぶすように
## v4.13.0
- feat: 派生スクリプト xx@@@yy.sn（@が３つ＋接尾辞形式）を開いた際、基底スクリプト xx.sn からの継承内容をインレイ表示するように
- 既知の問題
	- 起動直後にインレイ表示されない件。少しスクロールなどで表示更新をうながせば表示されるが……。
## v4.12.1
...
## v4.11.0
- feat: 画像最適化：素材ファイルの複数個ドロップにも対応
- feat: 音声最適化：素材ファイルの複数個ドロップにも対応
- fix: バッチ処理の起動高速化
## v4.10.7
- fix: フォルダごと追加・削除に対応、ファイルを含んでいる場合にも対応
	- fix: フォルダごと削除時、フォルダ内ファイルごとの削除イベントが発生しない件
- fix: フォルダごと削除時、暗号化フォルダ・prj_base に残る件
- fix: [... fn= label=] 系ホバーで同じ内容の【リンクを開く】が重複する場合がある件
- fix: 開発環境表示で npm バージョンの末尾に改行コードが入っていた件
- fix: 初期化処理が二度走る件
- fix: 内部エラー修正
## v4.10.6
...
- fix: テンプレ初回オープン時、更新確認処理をしないように
	- 【更新があります。【ベース更新】ボタンを押してください】を表示しない
- fix: ライブラリ更新
## v4.10.4
- fix: テンプレ初回オープン時、自動ビルド後に文法チェックを改めて行うように
## v4.10.3
...
## v4.9.8
- fix: 音声最適化：並列処理の同時実行数を最大10とするように（by p-queue@6.6.2）
	- 無制限の並列処理をすると、数百〜千個でフリーズするので
- fix: 画像最適化：並列処理の同時実行数を最大50とするように（by p-queue@6.6.2）
## v4.9.7
- fix: 画像最適化：breakpage.json などの置換に失敗する場合があった件
- fix: 画像最適化：置換する必要のない htm ファイル置換での無用な警告を抑制
## v4.9.6
...
- Win 動作確認済み
## v4.8.0
- feat: [macro]に snippet_ext 属性追加、スニペット候補に追加できる
	- 指定できる値は以下の通り（属性のコード補完で選択できる）
		- SP_GSM
		- SOUND
		- FONT
		- SCRIPT
	- 別々のマクロで追加された場合、降順ソートでタグより先に選択肢表示する（例：fg2,fg,lay）
		- 解析でマクロ定義に出会うタイミングに関係なく一定の選択肢表示をさせるため
		- あと fg2を先、fgを後にしたいという要請からも
## v4.7.0
