...
| 内容 | 判断材料 | 節 |
|---|---|---|
| **tmpwiz の画像を縮小**（9枚・2800KB） | 1000px で -1.2MB／800px で -1.6MB（vsix 4.1MB → 2.9／2.5MB）。表示は `col-12`＝パネル幅いっぱいなので、**Retina の全幅では甘くなる**。**加工は作者の領分** | §3.5 |
| **旧版同居の警告にリリースページへの導線を足す** | いまは「拡張機能ビューを開く」だけ。**手順を知りたい相手にこそ手順ページが要る**。1ボタン分 | §3.5 |

⚠️ **更新手順の専用ページは作らない。** 更新通知の「リリースページを開く」が
既に `env.openExternal(URL_EXT_RELEASES)` を呼んでおり（[src/ActivityBar.ts](src/ActivityBar.ts)）、
**リリースノートが手順書そのもの**になっている。

⚠️ **tmpwiz の画面は外に出せない。** `enableScripts: true` の webview で
プロジェクト名を `postMessage` で受けて検証・作成まで走る。
画像だけリモート化する案も、**オフラインで選べる価値**と
`img-src https:` を再申請前に増やす説明コストで見送り。
